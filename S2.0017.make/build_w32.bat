...
if "%COMPILER%" == "clang" goto Findclang
...
if "%DEBUG%" == "Y" set OUTDIR=.\clangDebug
if "%DEBUG%" == "Y" set LNKOUT=./clangDebug
...
if not "%COMPILER%" == "clang" call :Compile src\w32\compat\dirent
...
if "%COMPILER%" == "clang" goto clangCompile
...
if "%COMPILER%" == "clang" goto clangLink
...
%COMPILER% -Wl,--large-address-aware -mthreads -gdwarf-2 -g3 %OPTS% -o %LNKOUT%/%MAKE%.exe @%LNKOUT%/link.sc -Wl,--out-implib=%LNKOUT%/libgnumake-1.dll.a
@echo off
goto :EOF
...
if not "%COMPILER%" == "clang" set PKGMSC=--msvc-syntax
